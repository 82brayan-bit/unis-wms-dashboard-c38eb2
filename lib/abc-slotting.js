...
async function syncWmsData(ctx, scope, body) {
  ...
  const inboundActs = await wmsPostAllOptional(ctx, '/wms-bam/inventory-activity/search-by-paging', { customerId: scope.customerId, activityTypes: ['RECEIVE'], createdTimeFrom: from, createdTimeTo: to, pageSize: 100 }, 40, summary, 'inboundWarning');
  ...
  const outboundActs = await wmsPostAllOptional(ctx, '/wms-bam/inventory-activity/search-by-paging', { customerId: scope.customerId, activityTypes: ['PICK'], createdTimeFrom: from, createdTimeTo: to, pageSize: 100 }, 40, summary, 'outboundWarning');
